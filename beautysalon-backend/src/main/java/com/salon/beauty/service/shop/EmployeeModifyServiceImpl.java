...
public class EmployeeModifyServiceImpl implements EmployeeModifyService {
    ...
    public boolean employeeModify(EmployeeModifyRequestDTO employeeModifyRequestDTO) throws Exception {
        employeeModifyRequestDTO.setShop_id(sqlSession.getMapper(ShopIdMapper.class).shopId(SecurityContextHolder.getContext().getAuthentication().getName()));
        if(!isValidLengthEmployeeName(employeeModifyRequestDTO.getEmployee_name())) throw new IllegalArgumentException("직원 이름을 20글자 이내로 입력해주세요.");
        if(!isValidLengthEmployeePhone(employeeModifyRequestDTO.getEmployee_phone())) throw new IllegalArgumentException("직원 전화번호를 20글자 이내로 입력해주세요.");
        if(!isValidLengthEmployeeAddress(employeeModifyRequestDTO.getEmployee_address())) throw new IllegalArgumentException("직원 주소를 50글자 이내로 입력해주세요.");
